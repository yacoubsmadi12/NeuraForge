...
export function TextToImageTool({ subscription, refreshSubscription }: ToolProps) {
  ...
  const { t } = useLanguage();
  const toolId: ToolId = "text-to-image";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.imageUrl) {
        ...
        setError(t('dashboard.textToImage.error.general'));
      }
    } catch (err: any) {
      setError(err.message || t('dashboard.textToImage.error.general'));
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Card className="border-none shadow-none">
      <CardHeader>
        <CardTitle className="font-headline">{t('dashboard.textToImage.title')}</CardTitle>
        <CardDescription>{t('dashboard.textToImage.description')}</CardDescription>
      </CardHeader>
      <CardContent>
        {showUpgradeAlert && (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2 mb-4">
          <Input
            ...
            placeholder={t('dashboard.textToImage.placeholder')}
            disabled={isLoading || showUpgradeAlert}
            className="flex-grow"
          />
          <Button type="submit" disabled={isLoading || !prompt || showUpgradeAlert}>
            {isLoading ? t('dashboard.textToImage.button.loading') : t('dashboard.textToImage.button.default')}
        ...
        <div className="aspect-square w-full bg-card rounded-lg flex items-center justify-center overflow-hidden border relative group">
          {isLoading ? (
            ...
            <div className="text-center text-muted-foreground p-4">
              ...
              <p>{t('dashboard.textToImage.initialState')}</p>
            </div>
          )}
        </div>
      </CardContent>
